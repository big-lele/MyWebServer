...
//ToStr std::string operator() (const T&)  //实现复杂的类型转化
template<class T, class FromStr = LexicalCast<std::string, T>
                , class ToStr = LexicalCast<T, std::string> > // 支持序列化和反序列化
...
public:
	...
	std::string toString() override{
	        try{
		        //return boost::lexical_cast<std::string>(m_val); //只能支持简单的类型转换
			return ToStr()(m_val);
	...
	void addListener(uint64_t key, on_change_cb cb){ // 监听函数
	        m_cbs[key] = cb;
	}
...
private:
	T m_val;
	//变更回调函数组,因为function没有比较函数，不方便删除
